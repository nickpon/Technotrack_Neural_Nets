...
class CarvanaDataset(dt.Dataset):
    ...
    def make_batch(self, batch_size=128):
        indexes = np.random.choice(range(0, len(self.files)), size=batch_size, replace=False)
        self.files = [self.files[i] for i in indexes]
        self.files.sort()
        self.mask_files = [self.mask_files[i] for i in indexes]
        self.mask_files.sort()
